 ...
 * #L%
 */
package io.github.prolobjectlink.prolog.zprolog.sample;
...
import io.github.prolobjectlink.prolog.zprolog.PrologBaseTest;

public class Company extends PrologBaseTest {
	...
	public void test02() {
		...
		query = engine
				.query("employee(Name,Dpto,Scale),department(Dpto,DepartmentName),salary(Scale,Money),Money < 2000.");
		assertTrue(query.hasSolution());
		assertArrayEquals(expecteds2, query.oneSolution());
		query.dispose();
	...
	public void test03() {
		...
		query = engine.query(
				"employee(Name,Dpto,Scale),department(Dpto,DepartmentName),salary(Scale,Money),Money > 3000,Money < 4500.");
		assertTrue(query.hasSolution());
		assertArrayEquals(expecteds5, query.oneSolution());
		query.dispose();
	...
	public void test04() {
		...
		query = engine
				.query("employee(Name,Dpto,Scale),department(Dpto,DepartmentName),salary(Scale,Money),Money > 3000.");
		assertTrue(query.hasSolution());
		assertArrayEquals(expecteds4, query.oneSolution());
		query.dispose();
	...
	public void test05() {
		...
		query = engine.query("employee(Name,_,_),salary(_,_).");
		assertTrue(query.hasSolution());
		assertArrayEquals(new PrologTerm[] { mcardon }, query.oneSolution());
		query.dispose();
	...
	@Test
	public void test09() {
		query = engine.query("employee(Name,Dpto,Scale).");
		Map<String, PrologTerm>[] allSolutionMap = query.allVariablesSolutions();
		PrologBaseTest.printVariablesSolution(allSolutionMap);
		query.dispose();
	}

	@Test
	public void test10() {
		PrologQuery query = engine.query("employee(Name,Dpto,Scale),salary(Scale,Money),Money>2000.");
		while (query.hasMoreSolutions()) {
			Map<String, PrologTerm> solutionMap = query.nextVariablesSolution();
			PrologBaseTest.printVariablesSolution(solutionMap);
		}
		query.dispose();
	}
	...
	@Test
	public void test11() {
		PrologQuery query = engine.query("employee(Name,Dpto,Scale).");
		Map<String, PrologTerm> solutionMap[] = query.nVariablesSolutions(3);
		PrologBaseTest.printVariablesSolution(solutionMap);
		query.dispose();
	}
